...
│       ├── api/
│       │   └── storage.py         # JSON-персистентность
│       ├── logic/
│       │   └── service.py         # Модели, бизнес-логика, расчёты
│       ├── ui/
│       │   └── budget_view.py     # Виджет с четырьмя вкладками
│       └── module.py
│
├── ui/
│   ├── themes/
...
- Список последних 10 транзакций в порядке добавления с цветовой разметкой доходов и расходов.
...
- Нарисована через `QPainter` без внешних зависимостей.
...
**Данные** сохраняются в `budget_data.json` рядом с модулем и не теряются между сессиями.
...
NavigationManager              modules/*/module.py
  ...
  │  slot 2–4   │ PlaceholderModule.create_widget()
  └─────────────┘
```
