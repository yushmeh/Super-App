...
### ✅ Утилита 5 — МедиаТрекер (`media_tracker`)
Личный каталог просмотренных фильмов, сериалов, прочитанных книг и пройденных игр с оценками.

**Вкладка «Библиотека»**
- Карточки всех записей с фильтрами по типу (фильм / сериал / книга / игра) и статусу (запланировано / в процессе / завершено).
- Оценка 1–10 отображается в виде звёзд (шаг 0.5) с числовым значением.
- Заметки с кратким превью на карточке.
- Создание, редактирование и удаление записей.

**Вкладка «Статистика»**
- Карточки: всего записей, завершено в текущем году, средняя оценка.
- Топ-5 по оценке с полным отображением рейтинга в звёздах.

**Вкладка «Диаграммы»**
- Две donut-диаграммы: распределение по типам медиа и по статусам.
- Легенда с количеством записей по каждой категории.

При завершении записи (`completed`) дата завершения фиксируется автоматически.

**Данные** сохраняются в `media.db` (SQLite) рядом с модулем.
...
NavigationManager              modules/*/module.py
  ...
  │  slot 4     │ MediaTrackerModule.create_widget()
  └─────────────┘
```
